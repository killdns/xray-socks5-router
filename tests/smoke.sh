...
docker run -d \
  ...
  --ip 198.51.100.20 \
  alpine:3.24 \
  sh -c 'while true; do printf "HTTP/1.1 200 OK\r\nContent-Length: 25\r\nConnection: close\r\n\r\nxray-socks5-router-smoke\n" | nc -l -p 8080; done & while true; do nc -u -l -p 9998 -e cat; done & while true; do nc -u -l -p 9999 -e cat; done' \
  >/dev/null
...
[ -n "$detected_input" ] && [ -n "$detected_output" ] && \
  [ "$detected_input" != "$detected_output" ] || {
...
printf '%s\n' "Ordinary two-network L3 TCP/UDP and SOCKS5 TCP/UDP smoke tests passed"
